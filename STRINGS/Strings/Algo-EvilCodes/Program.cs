...
namespace AhoCorasick
{
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                trie.Add(Console.ReadLine().ToUpper());
            }
            ...
            text = text.ToUpper();
            // find words
            foreach (var match in trie.Find(text))
            {
                int index = match.Item2 - match.Item1.Length + 1;
                int len = match.Item1.Length;
                if(index % 2 != 0)
                {
                    continue;
                }
            ...
            Console.WriteLine(new string(text1));
        }
    }
        ...
        public IEnumerable<Tuple<TValue, int>> Find(IEnumerable<T> text)
        {
            ...
            foreach (T c in text)
            {
                ...
                for (var t = node; t != root; t = t.Fail)
                {
                    ...
                    {
                        yield return new Tuple<TValue, int>(value, indexCounter);
                    }
                }
            }
        }
